        ...
        <form action="{{ route('admin.plans.edit', $plan->id) }}" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select name="features[link_disabling]" id="i-features-link-deactivation" class="custom-select{{ $errors->has('features.link_disabling') ? ' is-invalid' : '' }}">
                    @foreach([1 => __('On'), 0 => __('Off')] as $key => $value)
                        <option value="{{ $key }}" @if((old('features.link_disabling') !== null && old('features.link_disabling') == $key) || ($plan->features->link_disabling == $key && old('features.link_disabling') == null)) selected @endif>{{ $value }}</option>
                    @endforeach
                </select>
